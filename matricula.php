...
if (isset($_POST['Envio'])) {
    ...
    $comando = "c:/xampp/apache/bin/openssl smime -verify -in $nombreFirma";
    $comando .= " -inform PEM -binary -content $nombreTexto";
    $comando .= " -CAfile c:/xampp/apache/conf/ssl.crt/CAMIW.crt";

    exec($comando . " 2>&1", $salida, $res);
    if ($res == 0) {
        echo "<CENTER> <H2>FIRMA CORRECTA EN LOS DATOS</H2><BR>";
        ...
        if ($arrayCert['subject']['CN'] === $_SESSION['user']) {
            echo "<CENTER><H2>FIRMA COMPLETAMENTE CORRECTA</H2></CENTER>";
        } else {
            echo "<CENTER><H4><FONT color=red>Usuario incorrecto. La firma debe realizarla el usuario autentitado".$_SESSION['user']."</FONT></H4></CENTER>";
            echo "<CENTER><H4><FONT color=red>No se ha realizado la matrícula</FONT></H4></CENTER>";
            exit();
        }
    } else {
        echo "<CENTER> <H2><FONT color=red> FIRMA INCORRECTA</FONT></H2></CENTER>";
